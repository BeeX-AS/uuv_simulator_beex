...
#else
    gazebo::event::Events::DisconnectWorldUpdateBegin(this->rosPublishConnection);
#endif
    this->rosNode->shutdown();
}

/////////////////////////////////////////////////
void FinROSPlugin::SetReference(const uuv_gazebo_ros_plugins_msgs::FloatStamped::ConstPtr &_msg) {
...
/////////////////////////////////////////////////
gazebo::common::Time FinROSPlugin::GetRosPublishPeriod() { return this->rosPublishPeriod; }
...
/////////////////////////////////////////////////
void FinROSPlugin::Reset() { this->lastRosPublishTime.Set(0, 0); }
...
void FinROSPlugin::Load(gazebo::physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    this->subReference = this->rosNode->subscribe<uuv_gazebo_ros_plugins_msgs::FloatStamped>(
            this->commandSubscriber->GetTopic(),
            10,
            boost::bind(&FinROSPlugin::SetReference, this, _1));

    this->pubState =
            this->rosNode->advertise<uuv_gazebo_ros_plugins_msgs::FloatStamped>(this->anglePublisher->GetTopic(), 10);
    ...
    this->pubFinForce = this->rosNode->advertise<geometry_msgs::WrenchStamped>(wrenchTopic, 10);

    std::stringstream stream;
    stream << _parent->GetName() << "/fins/" << this->finID << "/get_lift_drag_params";
    this->services["get_lift_drag_params"] =
            this->rosNode->advertiseService(stream.str(), &FinROSPlugin::GetLiftDragParams, this);

    gzmsg << "Fin #" << this->finID << " initialized" << std::endl
          << "\t- Link: " << this->link->GetName() << std::endl
          << "\t- Robot model: " << _parent->GetName() << std::endl
          << "\t- Input command topic: " << this->commandSubscriber->GetTopic() << std::endl
          << "\t- Output topic: " << this->anglePublisher->GetTopic() << std::endl;

    this->rosPublishConnection =
            gazebo::event::Events::ConnectWorldUpdateBegin(boost::bind(&FinROSPlugin::RosPublishStates, this));
}

/////////////////////////////////////////////////
void FinROSPlugin::RosPublishStates() {
    // Limit publish rate according to publish period
    if (this->angleStamp - this->lastRosPublishTime >= this->rosPublishPeriod) {
