...
/////////////////////////////////////////////////
UnderwaterObjectROSPlugin::UnderwaterObjectROSPlugin() { validateMachine(); }

/////////////////////////////////////////////////
UnderwaterObjectROSPlugin::~UnderwaterObjectROSPlugin() { this->rosNode->shutdown(); }
...
void UnderwaterObjectROSPlugin::Load(gazebo::physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    this->subLocalCurVel = this->rosNode->subscribe<geometry_msgs::Vector3>(
            _parent->GetName() + "/current_velocity",
            10,
            boost::bind(&UnderwaterObjectROSPlugin::UpdateLocalCurrentVelocity, this, _1));

    this->services["set_use_global_current_velocity"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_use_global_current_velocity",
            &UnderwaterObjectROSPlugin::SetUseGlobalCurrentVel,
            this);

    this->services["set_added_mass_scaling"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_added_mass_scaling",
            &UnderwaterObjectROSPlugin::SetScalingAddedMass,
            this);

    this->services["get_added_mass_scaling"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_added_mass_scaling",
            &UnderwaterObjectROSPlugin::GetScalingAddedMass,
            this);

    this->services["set_damping_scaling"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_damping_scaling",
            &UnderwaterObjectROSPlugin::SetScalingDamping,
            this);

    this->services["get_damping_scaling"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_damping_scaling",
            &UnderwaterObjectROSPlugin::GetScalingDamping,
            this);

    this->services["set_volume_scaling"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_volume_scaling",
            &UnderwaterObjectROSPlugin::SetScalingVolume,
            this);

    this->services["get_volume_scaling"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_volume_scaling",
            &UnderwaterObjectROSPlugin::GetScalingVolume,
            this);

    this->services["set_fluid_density"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_fluid_density",
            &UnderwaterObjectROSPlugin::SetFluidDensity,
            this);

    this->services["get_fluid_density"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_fluid_density",
            &UnderwaterObjectROSPlugin::GetFluidDensity,
            this);

    this->services["set_volume_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_volume_offset",
            &UnderwaterObjectROSPlugin::SetOffsetVolume,
            this);

    this->services["get_volume_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_volume_offset",
            &UnderwaterObjectROSPlugin::GetOffsetVolume,
            this);

    this->services["set_added_mass_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_added_mass_offset",
            &UnderwaterObjectROSPlugin::SetOffsetAddedMass,
            this);

    this->services["get_added_mass_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_added_mass_offset",
            &UnderwaterObjectROSPlugin::GetOffsetAddedMass,
            this);

    this->services["set_linear_damping_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_linear_damping_offset",
            &UnderwaterObjectROSPlugin::SetOffsetLinearDamping,
            this);

    this->services["get_linear_damping_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_linear_damping_offset",
            &UnderwaterObjectROSPlugin::GetOffsetLinearDamping,
            this);

    this->services["set_linear_forward_speed_damping_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_linear_forward_speed_damping_offset",
            &UnderwaterObjectROSPlugin::SetOffsetLinearForwardSpeedDamping,
            this);

    this->services["get_linear_forward_speed_damping_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_linear_forward_speed_damping_offset",
            &UnderwaterObjectROSPlugin::GetOffsetLinearForwardSpeedDamping,
            this);

    this->services["set_nonlinear_damping_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/set_nonlinear_damping_offset",
            &UnderwaterObjectROSPlugin::SetOffsetNonLinearDamping,
            this);

    this->services["get_nonlinear_damping_offset"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_nonlinear_damping_offset",
            &UnderwaterObjectROSPlugin::GetOffsetNonLinearDamping,
            this);

    this->services["get_model_properties"] = this->rosNode->advertiseService(
            _parent->GetName() + "/get_model_properties",
            &UnderwaterObjectROSPlugin::GetModelProperties,
            this);

    this->rosHydroPub["current_velocity_marker"] =
            this->rosNode->advertise<visualization_msgs::Marker>(_parent->GetName() + "/current_velocity_marker", 0);

    this->rosHydroPub["using_global_current_velocity"] =
            this->rosNode->advertise<std_msgs::Bool>(_parent->GetName() + "/using_global_current_velocity", 0);

    this->rosHydroPub["is_submerged"] =
            this->rosNode->advertise<std_msgs::Bool>(_parent->GetName() + "/is_submerged", 0);
}
...
void UnderwaterObjectROSPlugin::InitDebug(gazebo::physics::LinkPtr _link, gazebo::HydrodynamicModelPtr _hydro) {
    ...
    for (std::map<std::string, gazebo::transport::PublisherPtr>::iterator it = this->hydroPub.begin();
         it != this->hydroPub.end();
         ++it) {
        this->rosHydroPub[it->first] =
                this->rosNode->advertise<geometry_msgs::WrenchStamped>(it->second->GetTopic(), 10);
...
void UnderwaterObjectROSPlugin::PublishCurrentVelocityMarker() {
    ...
    if (this->flowVelocity.Length() > 0) {
        ...
        double pitch = std::atan2(
                this->flowVelocity.Z(),
                std::sqrt(std::pow(this->flowVelocity.X(), 2) + std::pow(this->flowVelocity.Y(), 2)));
...
bool UnderwaterObjectROSPlugin::SetUseGlobalCurrentVel(
    ...
    if (_req.use_global == this->useGlobalCurrent) {
        ...
        if (this->useGlobalCurrent) {
            ...
            gzmsg << this->model->GetName() << "::Using the current velocity under the namespace "
...
bool UnderwaterObjectROSPlugin::GetModelProperties(
    ...
    for (std::map<gazebo::physics::LinkPtr, gazebo::HydrodynamicModelPtr>::iterator it = models.begin();
         it != models.end();
         ++it) {
