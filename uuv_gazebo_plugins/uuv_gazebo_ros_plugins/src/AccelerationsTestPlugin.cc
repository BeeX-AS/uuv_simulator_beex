...
/////////////////////////////////////////////////
AccelerationsTestPlugin::AccelerationsTestPlugin() { validateMachine(); }
...
/////////////////////////////////////////////////
void AccelerationsTestPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
...
Eigen::Matrix3d Matrix3ToEigen(const ignition::math::Matrix3d &m) {
    Eigen::Matrix3d r;
    r << m(0, 0), m(0, 1), m(0, 2), m(1, 0), m(1, 1), m(1, 2), m(2, 0), m(2, 1), m(2, 1);
    return r;
}
...
void AccelerationsTestPlugin::Update(const common::UpdateInfo &_info) {
    ...
    Eigen::Vector6d gazebo_b_v_w_b = EigenStack(this->link->RelativeLinearVel(), this->link->RelativeAngularVel());

    // Velocities of this link in world frame
    Eigen::Vector6d gazebo_w_v_w_b = EigenStack(this->link->WorldLinearVel(), this->link->WorldAngularVel());


    // Accelerations of this link in world frame
    Eigen::Vector6d gazebo_w_a_w_b = EigenStack(this->link->WorldLinearAccel(), this->link->WorldAngularAccel());

    // Accelerations of this link in link frame
    Eigen::Vector6d gazebo_b_a_w_b = EigenStack(this->link->RelativeLinearAccel(), this->link->RelativeAngularAccel());
#else
    // Velocities of this link in link frame.
    Eigen::Vector6d gazebo_b_v_w_b =
            EigenStack(this->link->GetRelativeLinearVel().Ign(), this->link->GetRelativeAngularVel().Ign());

    // Velocities of this link in world frame
    Eigen::Vector6d gazebo_w_v_w_b =
            EigenStack(this->link->GetWorldLinearVel().Ign(), this->link->GetWorldAngularVel().Ign());


    // Accelerations of this link in world frame
    Eigen::Vector6d gazebo_w_a_w_b =
            EigenStack(this->link->GetWorldLinearAccel().Ign(), this->link->GetWorldAngularAccel().Ign());

    // Accelerations of this link in link frame
    Eigen::Vector6d gazebo_b_a_w_b =
            EigenStack(this->link->GetRelativeLinearAccel().Ign(), this->link->GetRelativeAngularAccel().Ign());
#endif
    ...
    R6_b_w_eigen << R_b_w_eigen, Eigen::Matrix3d::Zero(), Eigen::Matrix3d::Zero(), R_b_w_eigen;
...
void AccelerationsTestPlugin::Connect() {
    ...
    this->updateConnection =
            event::Events::ConnectWorldUpdateBegin(boost::bind(&AccelerationsTestPlugin::Update, this, _1));
}
}  // namespace gazebo
