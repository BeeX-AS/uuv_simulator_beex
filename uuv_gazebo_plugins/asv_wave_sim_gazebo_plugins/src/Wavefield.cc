...
public:
    void Recalculate() {
        ...
        for (size_t i = 0; i < this->number; ++i) {
            ...
            const TransformMatrix T(c, -s, s, c);
...
WaveParameters::WaveParameters() : data(new WaveParametersPrivate()) { this->data->Recalculate(); }
...
size_t WaveParameters::Number() const { return this->data->number; }

double WaveParameters::Angle() const { return this->data->angle; }

double WaveParameters::Scale() const { return this->data->scale; }

double WaveParameters::Steepness() const { return this->data->steepness; }

double WaveParameters::AngularFrequency() const { return this->data->angularFrequency; }

double WaveParameters::Amplitude() const { return this->data->amplitude; }

double WaveParameters::Period() const { return this->data->period; }

double WaveParameters::Phase() const { return this->data->phase; }

double WaveParameters::Wavelength() const { return this->data->wavelength; }

double WaveParameters::Wavenumber() const { return this->data->wavenumber; }

Vector2 WaveParameters::Direction() const { return this->data->direction; }
...
const std::vector<double> &WaveParameters::AngularFrequency_V() const { return this->data->angularFrequencies; }

const std::vector<double> &WaveParameters::Amplitude_V() const { return this->data->amplitudes; }

const std::vector<double> &WaveParameters::Phase_V() const { return this->data->phases; }

const std::vector<double> &WaveParameters::Steepness_V() const { return this->data->steepnesses; }

const std::vector<double> &WaveParameters::Wavenumber_V() const { return this->data->wavenumbers; }

const std::vector<Vector2> &WaveParameters::Direction_V() const { return this->data->directions; }
...
public:
    WavefieldPrivate(const std::array<double, 2> &_size, const std::array<size_t, 2> &_cellCount)
            : params(new WaveParameters()),
              size(_size),
...
Wavefield::Wavefield(const std::string &_name) : data(new WavefieldPrivate()) {
    // Grid
    this->data->initialGrid.reset(new Grid(this->data->size, this->data->cellCount));
    this->data->grid.reset(new Grid(this->data->size, this->data->cellCount));
...
std::shared_ptr<const Mesh> Wavefield::GetMesh() const { return this->data->grid->GetMesh(); }

std::shared_ptr<const Grid> Wavefield::GetGrid() const { return this->data->grid; }
...
std::shared_ptr<const WaveParameters> Wavefield::GetParameters() const { return this->data->params; }
...
void Wavefield::UpdateGerstnerWave(double _time) {
    ...
    for (auto &&it = std::make_pair(std::begin(initMesh.vertices()), std::begin(mesh.vertices()));
...
public:
    std::shared_ptr<const Grid> initWaterPatch;

    /// \brief A sample of the wavefield. This copy is updated.

public:
    std::shared_ptr<Grid> waterPatch;
};
...
WavefieldSampler::WavefieldSampler(std::shared_ptr<const Wavefield> _wavefield, std::shared_ptr<const Grid> _waterPatch)
...
std::shared_ptr<const Grid> WavefieldSampler::GetWaterPatch() const { return this->data->waterPatch; }
...
void WavefieldSampler::UpdatePatch() {
    ...
    for (auto &&vb = std::begin(target->vertices()); vb != std::end(target->vertices()); ++vb) {
        ...
        bool isFound = GridTools::FindIntersectionIndex(wavefieldGrid, origin.x(), origin.y(), cellIndex);
        if (!isFound) {
            // @DEBUG_INFO
        ...
        isFound = GridTools::FindIntersectionGrid(wavefieldGrid, origin, direction, cellIndex, point);
...
double WavefieldSampler::ComputeDepth(const Grid &_patch, const Point3 &_point) {
    // Calculate the depth
    Direction3 direction(0, 0, 1);
    Point3 wavePoint = CGAL::ORIGIN;
    std::array<size_t, 3> index;
    bool isFound = GridTools::FindIntersectionIndex(_patch, _point.x(), _point.y(), index);
    if (!isFound) {
        // @DEBUG_INFO
    ...
    isFound = GridTools::FindIntersectionGrid(_patch, _point, direction, index, wavePoint);
    if (!isFound) {
        // @DEBUG_INFO
        gzerr << "point:  " << _point << std::endl;
        // _patch.DebugPrint();
...
void WavefieldSampler::ComputeVelocities(
    ...
    auto solveVelocities = [=](auto x, auto wp, auto t, auto &v, auto &w, double z) {
        ...
        for (auto &&i = 0; i < n; ++i)  // NOLINT
        {
            ...
            const double k            = wp.k[i];
            const double depth_factor = exp(k * z);
            if (std::isnan(depth_factor) || std::isinf(depth_factor)) {
                v(0) = v(1) = v(2) = 0;
                w(0) = w(1) = w(2) = 0;
            }
            const double a = wp.a[i] * depth_factor;
        ...
        w(2) = 0;
        // std::cout<< N << " " << roll << " " << pitch << " " << std::endl << "dNormN "<< std::endl << dNormN <<
        // std::endl; std::cout<< "W" << std::endl << w << std::endl;
    ...
    // cppcheck-suppress constParameter
    auto solver =
            [=](auto &fdfunc, auto x0, auto p, auto t, auto &wp, auto tol, auto nmax, auto &v, auto &w, double z) {
                int n      = 0;
                double err = 1;
                double pz  = 0;
                auto xn    = x0;
                ...
                if (std::isnan(x0.norm()) || std::isinf(x0.norm())) {
    ...
    Eigen::Vector2d p2(_point.X(), _point.Y());
    if (std::isnan(p2.norm()) || std::isinf(p2.norm())) {
        v(0) = v(1) = v(2) = w(0) = w(1) = w(2) = 0;
        return;
    }
    // const double pz = solver(wave_fdf, p2, p2, time, wp, tol, nmax);
